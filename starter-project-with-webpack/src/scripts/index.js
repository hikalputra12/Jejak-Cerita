...
import { showFormattedDate } from './utils/index'; // Import showFormattedDate dari utils/index.js
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const updateAuthLinks = () => {
    const authLinksContainer = document.getElementById('auth-links'); // Menggunakan ID yang benar
    const navList = document.getElementById('nav-list');

    // Bersihkan link autentikasi yang ada di kedua tempat
    if (authLinksContainer) {
      authLinksContainer.innerHTML = '';
    }

    // Hapus item login/register yang ada dari navList jika ada
    ...
    if (UserAuth.isAuthenticated()) {
      ...
      if (navList) {
        ...
        if (!hasAddStory) {
          ...
          if (hasAbout) { // Sisipkan sebelum About
            navList.insertBefore(addStoryItem, hasAbout.parentElement);
          } else { // Atau cukup tambahkan
         ...
         if (!hasStories) {
          ...
          if (homeLink) { // Sisipkan setelah Home
      ...
      if (navList) {
        ...
        // Hapus "Tambah Cerita" dan "Semua Cerita" jika tidak login
  ...
  if (skipLink && mainContent) {
    skipLink.addEventListener('click', (event) => {
      ...
      mainContent.focus(); // Fokuskan ke area konten utama
    });
  }
});